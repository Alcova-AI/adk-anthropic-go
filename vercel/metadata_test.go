...
func TestParseMetadata(t *testing.T) {
	...
	metadata, ok := ParseMetadata(raw)
	if !ok {
		t.Fatal("ParseMetadata() did not find metadata")
	}
...
func TestParseMetadata_IgnoresUnknownOrMalformedData(t *testing.T) {
	for _, raw := range []string{"", `{}`, `{not json}`, `{"providerMetadata":{"other":{}}}`} {
		if metadata, ok := ParseMetadata(raw); ok {
			t.Fatalf("ParseMetadata(%q) = %+v, true", raw, metadata)
		}
	}
}
